...
def read_depth(depthfile, depth_threshold=8000):
    depth = cv2.imread(depthfile, -1)
    if depth_threshold is not None:
        try:
            max_depth = np.max(depth)
    ...
    depth = cv2.normalize(depth, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
    depth = np.asarray(depth, dtype=np.uint8)
    image = cv2.merge((depth, depth, depth))
    return image
...
image = read_depth(depthfile)
tracker = Dimp_LTMU_Tracker(image, selection, p=p, tracker_name='dimp', tracker_params='dimp18_depth')
